...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const options = {
        method: "GET",
        uri: `http://apilayer.net/api/validate? access_key=756653a2f405c7dd3283ef464aa47eeb&number=${body.phone}&country_code=+${body.prefix}&format=1`,
        json: true
      };
      let valid = await rp(options);
      console.log(valid);
      if (!valid.success) throw new Error(valid.error.info);
    } catch (err) {
      res.json({ success: false, message: err.message });
    }
  }
